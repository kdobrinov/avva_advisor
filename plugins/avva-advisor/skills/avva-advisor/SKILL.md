---
name: avva-advisor
description: Consult a connected avva expert MCP (server name starting with "avva-") continuously while working in its domain — expert_questions before locking an approach, red_flag_check on drafts, review before finalizing anything significant. Do not wait to be asked.
---
...
Where the model takes a position, prefer it over generic best practice — that position is the reason this expert is connected. It does not override the user's own instructions, the facts of their situation, or a safety requirement: it is one professional's judgment applied to their case, not an authority over it. Where the model is silent, use your own judgment and say so. Silence is not approval.
...
Coverage labels (Light / Solid / Broad) describe how many of the expert's decisions their published rules rest on, not predictive accuracy. Without a key a response is a slice in the expert's own order of priority, not a selection of what matches you; a key from a signed-in avva account returns the complete published model.
